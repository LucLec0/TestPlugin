...
function validateSetup(setup) {
  if (setup.players.length !== setup.playerCount) {
    return "Le nombre de joueurs édités ne correspond pas à la configuration.";
  }
  if (setup.tribes.length !== setup.tribeCount) {
    return "Le nombre de tribus éditées ne correspond pas à la configuration.";
  }
  if (setup.playerCount % setup.tribeCount !== 0) {
    return "Le nombre de joueurs doit être divisible par le nombre de tribus.";
  }
  ...
  const uneven = setup.tribes.some((tribe) => (perTribe[tribe.id] || 0) !== expectedPerTribe);
  if (uneven) {
    return "Chaque tribu doit contenir exactement le même nombre de joueurs au départ.";
  }
  if (setup.mergeAt >= setup.playerCount) {
    return "Le seuil de fusion doit être inférieur au nombre de joueurs initial.";
  }
  if (!setup.players.find((p) => p.id === setup.humanPlayerId)) {
    return "Choisis un joueur humain valide.";
  }
  return null;
}
...
function updateDivisibilityWarning() {
  ...
  el.divisibilityWarning.style.color = ok ? "#a7f3d0" : "#fecaca";
}
